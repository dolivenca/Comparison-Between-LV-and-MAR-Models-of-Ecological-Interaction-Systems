...
### Figure 3c original code http://127.0.0.1:16583/library/gauseR/html/lv_optim.html
###############################
...
# setwd("C:\\Users\\doliv\\OneDrive\\2_2019_America\\2020\\20200123_MAR\\Camparison_LV_MAR\\8_methods_of_ecology_and_evolution_2sub\\paper_figures")     # P51
# tiff("Fig3a.tiff", height = 7.5, width = 7.5, units = 'cm',compression = "lzw", res = 300)
par(mfrow=c(1,1))		#,mar=c(5, 6, 4, 2) + 0.1
plot(fig1data[,1], fig1data[,2], col=colorPallet[1], main='', xlab="Days", ylab="", cex.main=.8, cex.lab=1.5, cex.axis=1.5) #
...
points(2:(length(MARest_orig$MAR_TS_Est)+1),MARest1_smoothData$MAR_TS_Est,type='l',col='orange',lwd=3)
points(2:(length(MARest_orig$MAR_TS_Est)+1),MARest2_smoothData$MAR_TS_Est,type='l',col='khaki',lwd=3)
points(out_est1[,1],out_est1[,2],type='l',col=colorPallet[3],lwd=3)
mtext(expression(paste(italic('P. caudatum'))),side=3,line=1,cex=1.5)
mtext("std. volume",side=2,line=2.5,cex=1.5)
...
SSE1 = c(
	...
	sum( (fig1data[,2]-MARest1_smoothData$MAR_TS_Est)^2 ),
	sum( (fig1data[,2]-MARest2_smoothData$MAR_TS_Est)^2 )
	)



### steepest descent
# initPars_3a = c(a1 = 0.92288810, b11 = -0.00456388) 
   
OBJFUN_3a = function (initPars_3a)
	{
	### sum of squared errors for Fig3a data  

	tryCatch({	
		est1_mat_3a = Format_pars(truePars = initPars_3a)
		splineStart_3aOF = smoother_out$splines_est[1,2];names(splineStart) = 'x1'
		out_est_3aOF = try(solveLV(times = t_fig1, pars = est1_mat_3a, initState = splineStart_3aOF, equations = Equations),TRUE) # events = list(data = perturb) 
	
		error = 100000 * runif(1,.95,1.05)					# if the solver fail return a random high value for error
		if ( class(out_est_3aOF)[1]!="try-error" )			# check if solver worked
			{ 																																							
			if ( sum(is.nan(out_est_3aOF))==0 )				# check if solver worked by looking for NaNs												
				{
				plot(fig1data[,1],fig1data[,2], main='', xlab="Days", ylab="", cex.main=.8, cex.lab=1.5, cex.axis=1.5)	# open empty plot
				lines(timesq, Ntest,col="lightgrey",lwd=3)
				lines(out_est1[,1],out_est1[,2],col='blue',lwd=3)
				lines(out_est_3aOF[,1],out_est_3aOF[,2],lty=2,lwd=2,col="cyan")
				legend(13,100,legend=c('data','Muhlbauer est','ALVI-MI est','ALVI-MI + steepestDescent est'),lty = c(0,1,1,2),pch = c(1,NA,NA,NA),col = c('black','lightgrey','blue','cyan'),text.col=c('black','black','black','black'),bty = "n",cex=1,lwd=c(1,1,1,1))
	
				# cbind(out_est_3aOF,fig1data)
				error = sum( (fig1data[,2] - out_est_3aOF[,2])^2 )		# calculate error
				}
			}
		})
	return(error)		# return error	
	}
OBJFUN_3a(initPars_3a = c(a1 = 0.92288810, b11 = -0.00456388))

set.seed(3000)			# set seed for reproducable results
start_pars = c(a1 = 0.92288810, b11 = -0.00456388)
# start_pars = start_pars = start_pars + rnorm(length(start_pars),mean=0,sd=.2)		# mess the true pars to create a starting pars for the optimization, random sets of values will probalby fail 
...
# setwd("C:\\Users\\doliv\\OneDrive\\2_2019_America\\2020\\20200123_MAR\\Camparison_LV_MAR\\8_methods_of_ecology_and_evolution_2sub\\paper_figures")     # P51
# tiff("Fig3b.tiff", height = 15, width = 7.5, units = 'cm',compression = "lzw", res = 300)
par(mfrow=c(2,1))		 #,mar=c(5, 6, 4, 2) + 0.1  \n
main_lab = c( expression(paste(italic('P. caudatum'))),expression(paste(italic('P. aurelia'))) )
data_fig2 = mixturedata[,c(3,5,7)]
for (v in 2:3)	 
	{
	plot(data_fig2[,1], data_fig2[,v], col=colorPallet[1], main='', xlab="Days", ylab="", cex.main=.8 , cex.lab=1.5, cex.axis=1.5 )
	...
	points(2:24,MARest1_smoothData$MAR_TS_Est[,v-1],type='l',col='orange',lwd=3)
	points(2:24,MARest2_smoothData$MAR_TS_Est[,v-1],type='l',col='khaki',lwd=3)
	points(out_est1[,1], out_est1[,v], type='l', col=colorPallet[3], lwd=3)
	mtext(main_lab[v-1],side=3,line=1,cex=1.5)
	mtext("Std. Volume",side=2,line=2.5,cex=1.5)
	}
...
SEE2 = c(
	...
	sum( (fig2data[,2:3]-MARest1_smoothData$MAR_TS_Est)^2 ),
	sum( (fig2data[,2:3]-MARest2_smoothData$MAR_TS_Est)^2 )
	)
...
### steepest descent
# initPars_3c = c(a1 = 1.70705667, b11 = -0.03886794, b12 = -0.11360486, a2 = -1.27639378, b21 = 0.14275494, b22 = 0.01565327 ) 
   
OBJFUN_3c = function (initPars_3c)
	{
	### sum of squared errors for Fig3e data  

	tryCatch({	
		est1_mat_3c = Format_pars(truePars = initPars_3c)
		splineStart_3cOF = smoother_out$splines_est[1,2:3];names(splineStart) = c('Individuals_Prey', 'Individuals_Predator')
		out_est_3cOF = try(solveLV(times = seq(1,17,.1), pars = est1_mat_3c, initState = splineStart_3cOF, equations = Equations),TRUE) # events = list(data = perturb) 
	
		error = 100000 * runif(1,.95,1.05)					# if the solver fail return a random high value for error
		if ( class(out_est_3cOF)[1]!="try-error" )			# check if solver worked
			{ 																																							
			if ( sum(is.nan(out_est_3cOF))==0 )				# check if solver worked by looking for NaNs												
				{
				par(mfrow=c(2,1))		# ,mar=c(5, 6, 4, 2) + 0.1
				main_lab = c( expression(paste(italic('P. caudatum'), " (Prey)")),expression(paste(italic('D. nasutum'), " (Predator)")) )
				for (v in 2:3)
					{
					plot(fig3data[,1], fig3data[,v], ylim=c(0,60), col=colorPallet[1], main='', xlab='Days', ylab="", cex.main=.8, cex.lab=1.5, cex.axis=1.5)
					points(out_fig3data[,1],out_fig3data[,2],type='l',col=colorPallet[2],lwd=3)
					points(out_est1[,1],out_est1[,v],type='l',col=colorPallet[3],lwd=3)
					lines(out_est_3cOF[,1],out_est_3cOF[,v],lty=2,col='cyan',lwd=2)
					...
					mtext(main_lab[v-1],side=3,line=1,cex=1.5)
					mtext("Individuals",side=2,line=2.5,cex=1.5)
					}

				# cbind(out_est_3cOF[out_est_3cOF[,1]%in%fig3data[,1],],fig3data)
				error = sum( (fig3data[,2] - out_est_3cOF[out_est_3cOF[,1]%in%fig3data[,1],2])^2 ) + sum( (fig3data[,3] - out_est_3cOF[out_est_3cOF[,1]%in%fig3data[,1],3])^2 )		# calculate error
				}
			}
		})
	return(error)		# return error	
	}
OBJFUN_3c(initPars_3c = c(a1 = 1.70705667, b11 = -0.03886794, b12 = -0.11360486, a2 = -1.27639378, b21 = 0.14275494, b22 = 0.01565327 ))

set.seed(3000)			# set seed for reproducable results
start_pars = c(a1 = 1.70705667, b11 = -0.03886794, b12 = -0.11360486, a2 = -1.27639378, b21 = 0.14275494, b22 = 0.01565327 )
# start_pars = start_pars = start_pars + rnorm(length(start_pars),mean=0,sd=.2)		# mess the true pars to create a starting pars for the optimization, random sets of values will probalby fail 
...
opt						# optimization summary
start_pars = opt$par			# define new starting pars
OBJFUN_3c(initPars_3c = opt$par)
legend(8,65,legend=c('data','Muhlbauer est','ALVI-MI est','ALVI-MI + steepestDescent est'),lty = c(0,1,1,2),pch = c(1,NA,NA,NA),col = c('black','lightgrey','blue','cyan'),text.col=c('black','black','black','black'),bty = "n",cex=1,lwd=c(1,1,1,1))

pars_ALVIMI_steepest = opt$par
est1_mat_ALVIMI_steepest = Format_pars(truePars = pars_ALVIMI_steepest)
splineStart_ALVIMI_steepest = smoother_out$splines_est[1,2:3];names(splineStart) = c('Individuals_Prey', 'Individuals_Predator')
out_est_ALVIMI_steepest = try(solveLV(times = seq(1,17,.1), pars = est1_mat_ALVIMI_steepest, initState = splineStart_ALVIMI_steepest, equations = Equations),TRUE) # events = list(data = perturb) 


# setwd("C:\\Users\\doliv\\OneDrive\\2_2019_America\\2020\\20200123_MAR\\Camparison_LV_MAR\\10_PLOS_ONE\\paper_figures")     # P51
# tiff("Fig3c.tiff", height = 15, width = 7.5, units = 'cm',compression = "lzw", res = 300)
par(mfrow=c(2,1))		# ,mar=c(5, 6, 4, 2) + 0.1
main_lab = c( expression(paste(italic('P. caudatum'), " (Prey)")),expression(paste(italic('D. nasutum'), " (Predator)")) )
for (v in 2:3)
	{
	...
	points(MARest1_smoothData$MAR_TS_Est[,v-1],type='l',col='orange',lwd=3)
	lines(out_est_ALVIMI_steepest[,1],out_est_ALVIMI_steepest[,v],col='red',lwd=3)
	points(MARest2_smoothData$MAR_TS_Est[,v-1],type='l',col='khaki',lwd=3)
	points(out_est1[,1],out_est1[,v],type='l',col=colorPallet[3],lwd=3)
	#if (v==2) { legend(7,65, c('data','Muhlbauer est','Slope est','MAR','MAR logTrans'), col=col=colorPallet,pch=c(1,NA,NA,NA,NA), lwd=c(NA,3,3,3,3), lty=c(NA,1,1,1,1),bty = "n",cex=1) }
	mtext(main_lab[v-1],side=3,line=1,cex=1.5)
	mtext("Individuals",side=2,line=2.5,cex=1.5)
...
SSE3 = c(
	...
	sum( (fig3data[,2:3]-MARest1_smoothData$MAR_TS_Est)^2 ),
	sum( (fig3data[,2:3]-MARest2_smoothData$MAR_TS_Est)^2 )
	)
...
plot(fig4data[,1], fig4data[,2], col=colorPallet[1], main='', xlab='Year', ylab='Individuals', cex.main=.8, cex.lab=1.5, cex.axis=1.5)
...
points(1959:1992,MARest1_smoothData$MAR_TS_Est[,1],type='l',col='orange',lwd=3)
points(1959:1992,MARest2_smoothData$MAR_TS_Est[,1],type='l',col='khaki',lwd=3)
points(out_est1[,1],out_est1[,2],col=colorPallet[3],type='l',lwd=3)
mtext('Wolves',side=3,line=1,cex=1.5)

plot(fig4data[,1], fig4data[,3], col=colorPallet[1], main='', xlab='Year', ylab='Individuals', cex.main=.8, cex.lab=1.5, cex.axis=1.5)
...
points(1959:1992,MARest1_smoothData$MAR_TS_Est[,2],type='l',col='orange',lwd=3)
points(1959:1992,MARest2_smoothData$MAR_TS_Est[,2],type='l',col='khaki',lwd=3)
points(out_est1[,1],out_est1[,3],type='l',col=colorPallet[3],lwd=3)
mtext('Moose',side=3,line=1,cex=1.5)
# dev.off()

# tiff("Fig3d2.tiff", height = 15, width = 7.5, units = 'cm',compression = "lzw", res = 300)
par(mfrow=c(2,1))		#,mar=c(5, 6, 4, 2) + 0.1   
plot(fig4data[,1],fig4data[,4],col=colorPallet[1],main='', xlab='Year', ylab='tree growth', cex.main=.8, cex.lab=1.5, cex.axis=1.5)
...
points(1959:1992,MARest1_smoothData$MAR_TS_Est[,3],type='l',col='orange',lwd=3)
points(1959:1992,MARest2_smoothData$MAR_TS_Est[,3],type='l',col='khaki',lwd=3)
points(out_est1[,1],out_est1[,4],type='l',col=colorPallet[3],lwd=3)
mtext('Fir tree rings',side=3,line=1,cex=1.5)

plot(1,1,col='white',xlab='',ylab='',axes = FALSE)
legend(.6,1.3,legend = c('data','Mühlbauer est','ALVI-MI method','MAR no transform','MAR log transform','MAR no transform with smoothing','MAR log transform with smoothing','ALVI-MI + steepest descent'),lty = c(NA,1,1,1,1,1,1,1),lwd=c(NA,3,3,3,3,3,3,3),pch = c(1,NA,NA,NA,NA,NA,NA,NA),col = c(colorPallet,'orange','khaki','red'),bty = "n",cex=.5) # ,
# dev.off()
...
SSE4 = c(
	...
	sum( (fig4data[,2:4]-MARest1_smoothData$MAR_TS_Est)^2 ),
	sum( (fig4data[,2:4]-MARest2_smoothData$MAR_TS_Est)^2 )
	)
...
# setwd("C:\\Users\\doliv\\OneDrive\\2_2019_America\\2020\\20200123_MAR\\Camparison_LV_MAR\\8_methods_of_ecology_and_evolution_2sub\\paper_figures")     # P51
# tiff("Fig3e.tiff", height = 15, width = 7.5, units = 'cm',compression = "lzw", res = 300)
par(mfrow=c(2,1))		#,mar=c(5, 6, 4, 2) + 0.1   \n
plot(fig5data[,1],fig5data[,2],ylim=c(0,1300),col=colorPallet[1],main = expression(paste(italic('E. sexmaculatus'))),xlab='Week',ylab="",cex.main=1.5,cex.lab=1.5,cex.axis=1.5)
...
points(MARest1_smoothData$MAR_TS_Est[,1],type='l',col='orange',lwd=3)
points(MARest2_smoothData$MAR_TS_Est[,1],type='l',col='khaki',lwd=3)
points(out_est1[,1],out_est1[,2],type='l',col=colorPallet[3],lwd=3)
mtext("(Prey)",side=3,line=.4,cex=1.5)
mtext("Individuals",side=2,line=2.5,cex=1.5)
...
plot(fig5data[,1], fig5data[,3],col=colorPallet[1],ylim=c(0,30),main = expression(paste(italic("T. occidentalis"))), xlab='Week', ylab="", cex.main=1.5, cex.lab=1.5, cex.axis=1.5)
...
points(MARest1_smoothData$MAR_TS_Est[,2],type='l',col='orange',lwd=3)
points(MARest2_smoothData$MAR_TS_Est[,2],type='l',col='khaki',lwd=3)
points(out_est1[,1],out_est1[,3],type='l',col=colorPallet[3],lwd=3)
mtext("(Predator)",side=3,line=.4,cex=1.5)
mtext("Individuals",side=2,line=2.5,cex=1.5)
...
SSE5 = c(
	...
	sum( (fig5data[,2:3]-MARest1_smoothData$MAR_TS_Est)^2 ),
	sum( (fig5data[,2:3]-MARest2_smoothData$MAR_TS_Est)^2 )
	)



### steepest descent
# initPars_3e = c(a1 = 1.114803e-01, b11 = 2.683502e-05, b12 = -2.960126e-02, a2 = -8.000650e-01, b21 = 2.507228e-03, b22 = -3.143716e-02 ) 

OBJFUN_3e = function (initPars_3e)
	{
	### sum of squared errors for Fig3e data  

	tryCatch({	
		est1_mat_3e = Format_pars(truePars = initPars_3e)
		splineStart_3eOF = smoother_out$splines_est[1,2:3];names(splineStart) = c("x1", "x2")
		out_est_3eOF = try(solveLV(times = seq(1,60,.1), pars = est1_mat_3e, initState = splineStart_3eOF, equations = Equations),TRUE) # events = list(data = perturb) 
	
		error = 100000 * runif(1,.95,1.05)					# if the solver fail return a random high value for error
		if ( class(out_est_3eOF)[1]!="try-error" )			# check if solver worked
			{ 																																							
			if ( sum(is.nan(out_est_3eOF))==0 )				# check if solver worked by looking for NaNs												
				{
				par(mfrow=c(2,1))		#,mar=c(5, 6, 4, 2) + 0.1   \n
				plot(fig5data[,1],fig5data[,2],ylim=c(0,1300),col=colorPallet[1],main = expression(paste(italic('E. sexmaculatus'))),xlab='Week',ylab="",cex.main=1.5,cex.lab=1.5,cex.axis=1.5)
				points(out_fig5data[,1],out_fig5data[,2],type='l',col=colorPallet[2],lwd=3)
				points(out_est1[,1],out_est1[,2],type='l',col=colorPallet[3],lwd=3)
				lines(out_est_3eOF[,1],out_est_3eOF[,2],lty=2,lwd=2,col='cyan')
				mtext("(Prey)",side=3,line=.4,cex=1.5)
				mtext("Individuals",side=2,line=2.5,cex=1.5)
				#legend(0,1400,legend = c('data','Muhlbauer est','Slope est','MAR','MAR logTrans'), pch = c(1,NA,NA,NA,NA), lty = c(NA,1,1,1,1),col=colorPallet, bty = "n", lwd=c(NA,3,3,3,3),cex=.7) 
				...
				points(out_fig5data[,1],out_fig5data[,3],type='l',col=colorPallet[2],lwd=3)
				points(out_est1[,1],out_est1[,3],type='l',col=colorPallet[3],lwd=3)
				lines(out_est_3eOF[,1],out_est_3eOF[,3],lty=2,lwd=2,col='cyan')
				mtext("(Predator)",side=3,line=.4,cex=1.5)
				mtext("Individuals",side=2,line=2.5,cex=1.5)
				#legend(0,30,legend = c('data','Muhlbauer est','Slope est','MAR','MAR logTrans'), pch = c(1,NA,NA,NA,NA), lty = c(NA,1,1,1,1),col=colorPallet, bty = "n", lwd=c(NA,3,3,3,3),cex=.7) 

				# cbind(out_est_3eOF[out_est_3eOF[,1]%in%fig5data[,1],],fig5data)
				error = sum( (fig5data[,2] - out_est_3eOF[out_est_3eOF[,1]%in%fig5data[,1],2])^2 ) / median(fig5data[,2]) + sum( (fig5data[,3] - out_est_3eOF[out_est_3eOF[,1]%in%fig5data[,1],3])^2 ) / median(fig5data[,3])		# calculate error
				}
			}
		})
	return(error)		# return error	
	}
OBJFUN_3e(initPars_3e = c(a1 = 1.114803e-01, b11 = 2.683502e-05, b12 = -2.960126e-02, a2 = -8.000650e-01, b21 = 2.507228e-03, b22 = -3.143716e-02 ))

set.seed(3000)			# set seed for reproducable results
start_pars = c(a1 = 1.114803e-01, b11 = 2.683502e-05, b12 = -2.960126e-02, a2 = -8.000650e-01, b21 = 2.507228e-03, b22 = -3.143716e-02 )
# start_pars = start_pars = start_pars + rnorm(length(start_pars),mean=0,sd=.2)		# mess the true pars to create a starting pars for the optimization, random sets of values will probalby fail 
opt = optim(				# optimization
	par = start_pars, 
	fn = OBJFUN_3e,
...
opt						# optimization summary
start_pars = opt$par			# define new starting pars
OBJFUN_3e(initPars_3e = opt$par)
legend(40,35,legend=c('data','Muhlbauer est','ALVI-MI est','ALVI-MI + steepestDescent est'),lty = c(0,1,1,2),pch = c(1,NA,NA,NA),col = c('black','lightgrey','blue','cyan'),text.col=c('black','black','black','black'),bty = "n",cex=.7,lwd=c(1,1,1,1))
...
points(out_fig4data[,1],out_fig4data[,2],type='l',col=colorPallet[2],lwd=3)
points(out_est1[,1],out_est1[,2],col=colorPallet[3],type='l',lwd=3)
points(out_est1_1[,1],out_est1_1[,2],col='red',type='l',lwd=3)
points(out_est1_2[,1],out_est1_2[,2],col='red',type='l',lwd=3)
...
points(out_est1_1[,1],out_est1_1[,3],col='red',type='l',lwd=3)
points(out_est1_2[,1],out_est1_2[,3],col='red',type='l',lwd=3)
...
points(out_est1_1[,1],out_est1_1[,4],col='red',type='l',lwd=3)
points(out_est1_2[,1],out_est1_2[,4],col='red',type='l',lwd=3)

plot(1,1,col='white',xlab='',ylab='',axes = FALSE)
legend(.6,1.3,legend = c('data','Mühlbauer est','ALVI-MI method - all years','ALVI-MI method - minus 1981/82','MAR no transform','MAR log transform','MAR no transform with smoothness','MAR log transform with smoothness'),lty = c(NA,1,1,1,1,1,1,1),lwd=c(NA,3,3,3,3,3,3,3),pch = c(1,NA,NA,NA,NA,NA,NA,NA),col = c(colorPallet[1:3],'red',colorPallet[4:5],'orange','khaki'),bty = "n",cex=.7) # ,
# dev.off()
...
sum( (fig4data[1:22,2:4]-out_est1_1[,2:4])^2 ) + sum( (fig4data[25:34,2:4] - out_est1_2[,2:4])^2 )
