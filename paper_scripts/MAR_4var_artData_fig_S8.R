...
# setwd('C:\\Users\\doliv\\OneDrive\\2_2019_America\\2020\\20200123_MAR\\Camparison_LV_MAR\\4_getting_there\\paper_scripts')
...
# tiff("fig7.tiff", height = 20, width = 20, units = 'cm',compression = "lzw", res = 300)
par(mfrow=c(2,2),mar=c(5, 6, 4, 2) + 0.1)
for (iii in 1:4)
	{
	plot(MAR_artData1[,iii],type = 'l',lwd=3,col='lightgrey',xlim=c(0,dim(ni_store)[1]+.1*dim(ni_store)[1]),ylim=c(-10,80),xlab='t',ylab=varNames[iii])
...
MAR_artData1_clustered_means = MAR_artData1_forClust
...
# "100 % complete || DF = 12 12 12 12  ||  1 2 6 7 8 594.935  ||  1 2 6 7 8 594.935"

( est1 = LV_pars_finder(
				smooth_out = smoother_out,
				alg1_lm2 = 1, 
				data_sample_alg = c(1,2,6,7,8) # c(2, 3, 8, 14, 27)		#c(3, 7, 10, 14, 22)		
				) )
cbind(est1)
est1_mat = Format_pars(truePars = est1)
splineStart = smoother_out$splines_est[1,2:5];names(splineStart) = c('x1','x2','x3','x4')
...
#############################################################################
##### figure 4 - noisy and clustered MAR data 20% - alg1
...
# tiff("Fig_S8.tiff", height = 40, width = 20, units = 'cm',compression = "lzw", res = 300)
# Function to create a four independent color plot
par(mfcol=c(4,2))
mainLab = c('Noisy MAR Dataset',NA,NA,NA)
for (i in 1:4)
	{
	plot(MAR_artData1[,i],type = 'l',lwd=3,col=colorPallet[2],xlim=c(0,dim(ni_store)[1]+.1*dim(ni_store)[1]),ylim=c(-10,80),main=mainLab[i],xlab='t',ylab=varNames[i],cex.lab=1.5,cex.axis=1.5,cex.main=1.5)
	points(MAR_artData1_noisy[,i],pch=1,col=colorPallet[1])
	if (i==1) { legend(15,80,legend=c('MAR results','noisy MAR','slope est','MAR','MAR logTrans'),lty=c(1,NA,1,1,1),lwd=c(3,NA,3,3,3),pch=c(NA,1,NA,NA,NA),col=c(colorPallet[2],colorPallet[1],colorPallet[3],colorPallet[4],colorPallet[5]),bty = "n") }
...
for (i in 1:4)
	...
	if (i==1) { legend(15,80,legend=c('MAR results','replicate MAR','slope est','MAR','MAR logTrans'),lty=c(1,NA,1,1,1),lwd=c(3,NA,3,3,3),pch=c(NA,1,NA,NA,NA),col=c(colorPallet[2],colorPallet[1],colorPallet[3],colorPallet[4],colorPallet[5]),bty = "n") }
