...
######################################################################################################################################
### noisy data - choose 40 points at random and add a random normal of mean 0 and variance mean(var)*.5
...
for (i in 2:5)
	...
	for (ii in 1:dim(noisy_data3)[1])
		...
		while (temp1 < 0)
			{ 
			temp1 = noisy_data3[ii,i] + rnorm(1,0,.05 * mean(out1[,i]))
			}
...
# write.csv(noisy_data3,'noisy_data3_05.csv')
# noisy_data3 = read.csv('noisy_data3_05.csv')
# save(noisy_data3, file = "noisy_data3_05.Rdata")
# load(file = "noisy_data3_05.Rdata")
...
# tiff("figS1.tiff", height = 20, width = 20, units = 'cm',compression = "lzw", res = 300)
par(mfrow = c(2,2))
for (i in 2:5)
	{
	...
	points(out_est1[,1],out_est1[,i],type='l',col=varColor[i-1],lwd=3)
...
# SSE
sum( ( noisy_data3[,2:5] - out_est1[out_est1[,1]%in%noisy_data3[,1],2:5] )^2 )
...
( est1 = LV_pars_finder(
				smooth_out = smoother_out,
				alg1_lm2 = 1, 
				data_sample_alg = c(4,6,10,15,24)		#c(3, 7, 10, 14, 22)		
				) )
...
cbind(truePars1,est1,absDif = abs(truePars1-est1))
splineStart = smoother_out$splines_est[1,2:5];names(splineStart) = c('x1','x2','x3','x4')
out_est_alg1 = out_est1 = solveLV(times = t, pars = est1_mat, initState = splineStart, equations = Equations)
#noisy_data3[1,2:5]

# setwd("C:\\Users\\doliv\\OneDrive\\2_2019_America\\2020\\20200123_MAR\\Camparison_LV_MAR\\5_methods_of_ecology_and_evolution\\paper_figures")     # P51
# tiff("fig3_.tiff", height = 20, width = 20, units = 'cm',compression = "lzw", res = 300)
par(mfrow = c(2,2))
for (i in 2:5)
	{
	plot(out1[,1],out1[,i],pch=20,col='lightgrey',ylim=c(0,3),xlab='t',ylab=varNames[i-1],cex.lab=1.5,cex.axis=1.5,lwd=1)
...
##########
### find alg1 points for noisy_data3
...
# "100 % complete || DF = 11 11 11 11  ||  3 7 10 14 22 1.314  ||  3 7 10 14 22 0.297"
...
sum( ( noisy_data3[,2:5] - MARest1_orig[MARest1_orig[,1]%in%noisy_data3[,1],2:5] )^2 )
sum( ( noisy_data3[,2:5] - MARest1_log[MARest1_log[,1]%in%noisy_data3[,1],2:5] )^2 )
...
# setwd("C:\\Users\\doliv\\OneDrive\\2_2019_America\\2020\\20200123_MAR\\Camparison_LV_MAR\\5_methods_of_ecology_and_evolution\\paper_figures")     # P51
# tiff("fig3.tiff", height = 20, width = 20, units = 'cm',compression = "lzw", res = 300)
par(mfrow = c(2,2))
for (i in 2:5)
	{
	plot(out1[,1],out1[,i],pch=20,col='lightgrey',ylim=c(0,3),xlab='t',ylab=varNames[i-1],cex.lab=1.5,cex.axis=1.5,lwd=1)
	...
	points(out_est_alg1[,1],out_est_alg1[,i],type='l',col=varColor[i-1],lwd=3)
	points(MARest1$MAR_TS_Est[,i-1],col=varColor_light[i-1],type='l',lty=1)
	points(MARest2$MAR_TS_Est[,i-1],col=varColor_light[i-1],type='l',lty=2)
	if (i == 2) {legend(40,1.5,legend = c('data','sparse noisy data','smooth fun','Slopes est','MAR no transform','MAR log transform'),lty = c(0,0,1,1,1,2),pch = c(20,20,NA,NA,NA,NA),col = c('darkgrey',varColor[i-1],varColor[i-1],varColor[i-1],varColor_light[i-1],varColor_light[i-1]),text.col=c('black','black','black','black','black','black'),bty = "n",cex=1,lwd=c(NA,NA,1,3,1,1))}
	}
# dev.off()
...
### LVness1 - sparse and noisy data 3 - choose 40 points at random and add a random normal of mean 0 and variance mean(var)*.5
#############################################################################



#############################################################################
### LVness1 - sparse and noisy data 4 - nine points were chosen from the artificial data and for each point five observations were created multiplying it by a random normal of mean 1 and variance 0.5
...
for (i in 1:dim(data1_sparsed)[1])
	{
      for (ii in 1:datapoint_n)
		{
		noisy_data4 = rbind( noisy_data4, c(data1_sparsed[i,1], data1_sparsed[i,2:5] * rnorm(n = 4,mean = 1,sd = .05)) )
		}
	}
noisy_data4 = noisy_data4[2:dim(noisy_data4)[1],]

# setwd('C:\\Users\\doliv\\OneDrive\\2_2019_America\\2020\\20200123_MAR\\Camparison_LV_MAR\\5_methods_of_ecology_and_evolution\\paper_scripts')
# write.csv(noisy_data4,'noisy_data4_05.csv')
# noisy_data4 = read.csv('noisy_data4_05.csv')
# save(noisy_data4, file = "noisy_data4_05.Rdata")
# load(file = "noisy_data4_05.Rdata")
...
cbind(truePars1,est1,absDif = abs(truePars1-est1))
splineStart = smoother_out$splines_est[1,2:5];names(splineStart) = c('x1','x2','x3','x4')
out_est_noisy_data4 = out_est1 = solveLV(times = t, pars = est1_mat, initState = splineStart, equations = Equations)
#noisy_data3[1,2:5]
...
for (i in 2:5)
	...
	points(smoother_out$splines_est[,1],smoother_out$splines_est[,i],type='l',col=varColor[i-1])
	points(out_est_noisy_data4[,1],out_est_noisy_data4[,i],type='l',col=varColor[i-1],lwd=3)
	if (i == 2) {legend(35,1.2,legend = c('data','sparse noisy data','smooth','estimates'),lty = c(0,0,1,1),pch = c(20,20,NA,NA),col = c('darkgrey',varColor[i-1],varColor[i-1],varColor[i-1]),text.col=c('black','black','black'),bty = "n",cex=1,lwd=c(NA,NA,1,3))}
	}
# dev.off()
...
# SSE
SSE_folder = 0
for (i in unique(noisy_data4[,1]))
	{
	temp1 = matrix(out_est1[out_est1[,1]==i,2:5], nrow=dim(noisy_data4[noisy_data4[,1]==i,2:5])[1], ncol=4, byrow=TRUE)
	SSE_folder = SSE_folder + sum( ( noisy_data4[noisy_data4[,1]==i,2:5] - temp1 )^2 )
	}
SSE_folder
...
# SSE
MARest3_orig = cbind(1:dim(MARest3$MAR_TS_Est)[1],MARest3$MAR_TS_Est)
SSE_folder = 0
for (i in unique(noisy_data4[,1]))
	{
	temp1 = matrix(MARest3_orig[MARest3_orig[,1]==i,2:5], nrow=dim(noisy_data4[noisy_data4[,1]==i,2:5])[1], ncol=4, byrow=TRUE)
	SSE_folder = SSE_folder + sum( ( noisy_data4[noisy_data4[,1]==i,2:5] - temp1 )^2 )
	}
SSE_folder
...
# SSE
MARest4_orig = cbind(1:dim(MARest4$MAR_TS_Est)[1],MARest4$MAR_TS_Est)
SSE_folder = 0
for (i in unique(noisy_data4[,1]))
	{
	temp1 = matrix(MARest4_orig[MARest4_orig[,1]==i,2:5], nrow=dim(noisy_data4[noisy_data4[,1]==i,2:5])[1], ncol=4, byrow=TRUE)
	SSE_folder = SSE_folder + sum( ( noisy_data4[noisy_data4[,1]==i,2:5] - temp1 )^2 )
	}
SSE_folder
...
for ( i in 1:(dim(TS_noisy_data4)[2]-1) )
	{
	plot(out1[,i+1],type='l',col='lightgrey',lwd=3,ylab = varNames[i])
...
for (i in 2:5)
	...
	#points(smoother_out$splines_est[,1],smoother_out$splines_est[,i],type='l',col=varColor[i-1])
	points(out_est_noisy_data4[,1],out_est_noisy_data4[,i],type='l',col=varColor[i-1],lwd=3)
	points(MARest3$MAR_TS_Est[,i-1],col=varColor_light[i-1],type='l',lty=1)
	points(MARest4$MAR_TS_Est[,i-1],col=varColor_light[i-1],type='l',lty=2)
	if (i == 2) {legend(40,1.5,legend = c('data','sparse noisy data','smooth fun','Slopes est','MAR no transform','MAR log transform'),lty = c(0,0,1,1,1,2),pch = c(20,20,NA,NA,NA,NA),col = c('darkgrey',varColor[i-1],varColor[i-1],varColor[i-1],varColor_light[i-1],varColor_light[i-1]),text.col=c('black','black','black','black','black','black'),bty = "n",cex=1,lwd=c(NA,NA,1,3,1,1))}
	}
# dev.off()
...
#############################################################################
##### figure S3 - noisy and clustered data 5%
...
# setwd('C:\\Users\\doliv\\OneDrive\\2_2019_America\\2020\\20200123_MAR\\Camparison_LV_MAR\\6_methods_of_ecology_and_evolution_new_images\\paper_figures')
...
for (i in 2:5)
	{
	plot(out1[,1],out1[,i],pch=20,col=colorPallet[2],ylim=c(0,3),main=mainLab[i-1],xlab='t',ylab=varNames[i-1],cex.lab=1.5,cex.axis=1.5,lwd=1)
	points(noisy_data3[,1],noisy_data3[,i],pch=20,col=colorPallet[1]) 
	#points(smoother_out_alg1$splines_est[,1],smoother_out_alg1$splines_est[,i],type='l',col=varColor[i-1])
	points(out_est_alg1[,1],out_est_alg1[,i],type='l',col=colorPallet[3],lwd=3)
	...
	if (i == 2) {legend(40,1,legend = c('LV results','noisy dataset','Slopes est','MAR no transform','MAR log transform'),lty = c(0,0,1,1,1),pch = c(20,1,NA,NA,NA),col = c(colorPallet[2],colorPallet[1],colorPallet[3],colorPallet[4],colorPallet[5]),text.col=c('black','black','black','black','black'),bty = "n",cex=1,lwd=c(NA,NA,3,3,3))}
	}
mainLab = c('Replicate Dataset',NA,NA,NA)
for (i in 2:5)
	{
	plot(out1[,1],out1[,i],pch=20,col=colorPallet[2],ylim=c(0,3),main=mainLab[i-1],xlab='t',ylab=varNames[i-1],cex.lab=1.5,cex.axis=1.5,lwd=1)
	points(noisy_data4[,1],noisy_data4[,i],pch=20,col=colorPallet[1]) 
	#points(smoother_out$splines_est[,1],smoother_out$splines_est[,i],type='l',col=varColor[i-1])
	points(out_est_noisy_data4[,1],out_est_noisy_data4[,i],type='l',col=colorPallet[3],lwd=3)
	...
	if (i == 2) {legend(40,1,legend = c('LV results','replicate dataset','Slopes est','MAR no transform','MAR log transform'),lty = c(0,0,1,1,1),pch = c(20,1,NA,NA,NA),col = c(colorPallet[2],colorPallet[1],colorPallet[3],colorPallet[4],colorPallet[5]),text.col=c('black','black','black','black','black'),bty = "n",cex=1,lwd=c(NA,NA,3,3,3))}
	}
